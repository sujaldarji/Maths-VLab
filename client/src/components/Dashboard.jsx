...
const Dashboard = () => {
  ...
  // State for domain, search, difficulty
  ...
  // Filter topics based on domain, search, difficulty
  const displayedTopics = (topicsData[selectedDomain] || []).filter((topic) => {
    const matchesSearch = topic.title.toLowerCase().includes(debouncedSearch.toLowerCase());
    const matchesDifficulty = difficultyFilter === "All" || topic.difficulty === difficultyFilter;
    return matchesSearch && matchesDifficulty;
  ...
  // Navigate to the study page with the topic title as a query param
  // const handleTopicClick = (topicTitle,topicid) => {
  //   // e.g. /study?topic=Knowing%20Our%20Numbers
  //   navigate(`/study?topic=${encodeURIComponent(topicTitle,topicid)}`);
  // };
  const handleTopicClick = (topicTitle, topicId) => {
    console.log(`Navigating to study page with topic: ${topicTitle}, id: ${topicId}`); // Debug log
    navigate(`/study?topic=${encodeURIComponent(topicTitle)}&id=${encodeURIComponent(topicId)}`);
};
